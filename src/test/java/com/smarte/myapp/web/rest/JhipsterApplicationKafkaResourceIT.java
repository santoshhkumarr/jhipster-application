...
import com.smarte.myapp.config.KafkaProperties;
...
class JhipsterApplicationKafkaResourceIT {
    ...
    void setup() {
        ...
        JhipsterApplicationKafkaResource kafkaResource = new JhipsterApplicationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipster-application-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/jhipster-application-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
